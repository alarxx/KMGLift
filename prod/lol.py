...
# Алгоритм Хитрости: извлекаем край изображения
def Canny(img):
    # Gray scale
    def BGR2GRAY(img):
        b = img[:, :, 0].copy()
...
# Преобразование Хафа реализует обнаружение 20 прямых линий на изображении
def Hough_Line(edge, img):
    ...
    def voting(edge):
        ...
        # get index of edge
        # ind [0] - уточненная ордината, ind [1] - квалифицированная абсцисса
    ...
    def inverse_hough(hough, img):
        ...
        # np.ravel Уменьшить многомерный массив до 1 измерения
        # argsort Сортировать элементы массива от меньшего к большему и возвращать индекс
        # [:: - 1] Обратный порядок -> от большого к малому
        # [: 20] Первые 20
